...
if($accion == 'login'){
	...
	if(isset($params['usuario']) and isset($params['pass']) and $params['usuario'] != '' and $params['pass'] != ''){
		// LOGIN FORMULARIO
		$SQL = "SELECT id, nombre, apellido FROM usuarios WHERE mail = '".$params['usuario']."' AND clave = '".$params['pass']."' AND habilitado = 1 AND eliminado = 0";
...
if($accion == "listadoNoticias"){
	...
	if(mysqli_num_rows($RS) > 0){
		...
		while($n = mysqli_fetch_object($RS)){
			...
			if($n->imagen != ''){
				$noticias[$i]['contenido'] = strip_tags(substr(cambiarCaracteresEspeciales($n->texto),0,150));
				$noticias[$i]['imagen'] = $n->imagen;
			}else{
				$noticias[$i]['contenido'] = strip_tags(substr(cambiarCaracteresEspeciales($n->texto),0,300));
				$noticias[$i]['imagen'] = "";
			}
...
if($accion == 'noticiaInterna'){
	...
	**/
	...
	$noticia['legislatura'] = $RES['legislatura'];
